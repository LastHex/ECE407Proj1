...
if __name__ == "__main__":
	...
	try: 
		...
		#for i in range(Sample_per_lead):
		#for j in range(nbLeads):
		#	ecgSig[0][i] =  np.fromfile(f, dtype = np.int16, count = nbLeads)[0] 
...
# plotting ECG
		...
		y1 = np.zeros_like(ecgSig[0][:])
		y2 = np.zeros_like(ecgSig[1][:])
		y3 = np.zeros_like(ecgSig[2][:])
		inout(drv.Out(y1), drv.In(ecgSig[0][:]), (Resolution[0]), block=(400,1,1), grid=(1,1))
		inout(drv.Out(y2), drv.In(ecgSig[1][:]), (Resolution[1]), block=(400,1,1), grid=(1,1))
		inout(drv.Out(y3), drv.In(ecgSig[2][:]), (Resolution[2]), block=(400,1,1), grid=(1,1))
